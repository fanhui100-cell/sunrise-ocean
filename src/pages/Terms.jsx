    ...
    <div className="min-h-screen pt-20 lg:pt-24">
      ...
      >
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-2 tracking-tight">{t('terms.title')}</h1>
          <p className="text-white/40 text-sm mt-3">{t('terms.subtitle')} · {t('terms.last_updated_label')}: {t('terms.last_updated')}</p>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-navy-900 rounded-2xl p-6 mb-8 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-white/45 text-sm leading-relaxed">{t('terms.intro')}</p>
            </div>
          </div>
                  ...
                  {sec.body.split('\n').map((line, j) => {
                    ...
                    return (
                      <p key={j}>
                        {parts.map((part, k) =>
                          k % 2 === 1
                            ? <strong key={k} className="text-navy-900 font-semibold">{part}</strong>
                            : part
                        )}
                      </p>
                    );
          ...
          <div className="mt-8 bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-gray-400 text-xs mt-0.5">{t('terms.privacy_link_desc')}</p>
            </div>
            <Link
              to="/privacy"
